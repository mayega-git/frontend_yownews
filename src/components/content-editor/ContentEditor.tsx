...
const input: React.CSSProperties = { width: '100%', border: '1px solid var(--gray-200, #e5e7eb)', borderRadius: '8px', padding: '10px 12px', fontSize: '14px' };
...
export default function ContentEditor(
  ...
  const msgStyle = (kind: 'ok' | 'err' | 'warn'): React.CSSProperties => ({
    ...
    background: kind === 'ok' ? 'rgba(16,185,129,.1)' : kind === 'warn' ? 'rgba(245,158,11,.12)' : '#FEF2F2',
    color: kind === 'ok' ? '#059669' : kind === 'warn' ? '#B45309' : '#B91C1C',
  });

  return (
    <div style={{ maxWidth: '760px', display: 'flex', flexDirection: 'column', gap: '18px' }}>
      {message && <div style={msgStyle(message.kind)}>{message.text}</div>}

      <div>
        <label style={label}>Titre</label>
        <input style={input} value={title} onChange={(e) => setTitle(e.target.value)} placeholder={`Titre du ${config.noun}`} />
      </div>

      <div>
        <label style={label}>Description</label>
        <textarea style={{ ...input, minHeight: '70px', resize: 'vertical' }} value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Court résumé" />
      </div>
      ...
      <div>
        ...
        <select style={input} value={domain} onChange={(e) => setDomain(e.target.value)}>
          {domains.map((d) => <option key={d} value={d}>{d === 'NONE' ? 'Aucun — saisir mon domaine' : d}</option>)}
        </select>
        {showCustomDomain && (
          <input style={{ ...input, marginTop: '8px' }} value={customDomain} onChange={(e) => setCustomDomain(e.target.value)} placeholder="Votre domaine (ex. Cuisine)" />
        )}
      </div>
      ...
      {/* Champs spécifiques au type (corps TipTap pour blog, audio pour podcast, etc.) */}
      {config.extraFields}

      <div>
        <button type="button" disabled={saving} onClick={submit} style={{
          background: 'var(--primary)', color: '#fff', border: 'none', borderRadius: '10px',
          padding: '12px 22px', fontSize: '14px', fontWeight: 700, cursor: saving ? 'default' : 'pointer', opacity: saving ? 0.6 : 1,
