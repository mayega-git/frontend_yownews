...
import NewsletterContentModeration from './NewsletterContentModeration';
import { apiFetch } from '@/lib/api-client';

type Tab = 'newsletters' | 'gestion';
type GestionTab = 'requests' | 'content' | 'categories';
...
export default function NewslettersAdminWorkspace({ email }: { email: string }) {
  ...
  const [gestionTab, setGestionTab] = useState<GestionTab>('requests');
  ...
  const subTabStyle = (val: GestionTab): React.CSSProperties => ({
    padding: '7px 14px', fontSize: '13px', fontWeight: 600, cursor: 'pointer', borderRadius: '20px',
    border: `1px solid ${gestionTab === val ? 'var(--accent)' : 'var(--gray-200)'}`,
    background: gestionTab === val ? 'rgba(239,68,68,.08)' : '#fff',
    color: gestionTab === val ? 'var(--accent)' : 'var(--gray-600)',
  });

  return (
    <div>
      <div style={{ display: 'flex', gap: '4px', borderBottom: '1px solid var(--gray-200)', marginBottom: '24px' }}>
        <button type="button" style={tabStyle('newsletters')} onClick={() => setTab('newsletters')}>Newsletters</button>
        <button type="button" style={tabStyle('gestion')} onClick={() => setTab('gestion')}>Gestion des newsletters</button>
      </div>

      {tab === 'newsletters' ? (
        <NewsletterWorkspace email={email} />
      ) : (
        <div>
          <div style={{ display: 'flex', gap: '8px', marginBottom: '18px' }}>
            <button type="button" style={subTabStyle('requests')} onClick={() => setGestionTab('requests')}>Demandes de rédacteur</button>
            <button type="button" style={subTabStyle('content')} onClick={() => setGestionTab('content')}>Validation de contenu</button>
            <button type="button" style={subTabStyle('categories')} onClick={() => setGestionTab('categories')}>Catégories</button>
          </div>
          {gestionTab === 'requests' && <RedacteurRequestsModeration />}
          {gestionTab === 'content' && <NewsletterContentModeration />}
          {gestionTab === 'categories' && <CategoriesManager />}
        </div>
      )}
    </div>
  );
