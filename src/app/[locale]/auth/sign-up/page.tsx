...
const COUNTRY_CODES = [
  ...
  { flag: '🇸🇳', code: '+221', label: '🇸🇳 +221' },
  { flag: '🇨🇲', code: '+237', label: '🇨🇲 +237' },
  { flag: '🇰🇪', code: '+254', label: '🇰🇪 +254' },
  { flag: '🇳🇬', code: '+234', label: '🇳🇬 +234' },
  { flag: '🇬🇭', code: '+233', label: '🇬🇭 +233' },
  { flag: '🇲🇦', code: '+212', label: '🇲🇦 +212' },
  { flag: '🇩🇿', code: '+213', label: '🇩🇿 +213' },
  { flag: '🇹🇳', code: '+216', label: '🇹🇳 +216' },
  { flag: '🇨🇩', code: '+243', label: '🇨🇩 +243' },
  { flag: '🇧🇫', code: '+226', label: '🇧🇫 +226' },
  { flag: '🇲🇱', code: '+223', label: '🇲🇱 +223' },
  { flag: '🇬🇳', code: '+224', label: '🇬🇳 +224' },
...
export default function SignUpPage() {
  ...
  return (
    <div className="grid" style={{ gridTemplateColumns: '55% 45%', minHeight: '100vh' }}>
      <AuthLeftPanel
        ...
        headline={
          <>
            ...
            <span style={{ background: 'linear-gradient(90deg,#FF6B35,#FFB347)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>
              8&nbsp;500+
            </span>
      ...
      {/* Right panel */}
      <main
        className="bg-white flex items-start justify-center px-6 py-10 md:px-14 relative overflow-y-auto"
        role="main"
      >
        {/* Left border line */}
        <div
        ...
        <div className="w-full max-w-[420px] pt-2 pb-10">
          {/* Header */}
          <div className="mb-7">
          ...
          <div className="grid grid-cols-2 gap-2.5 mb-6">
            {[
              ...
              <button key={label} type="button" disabled className="flex items-center justify-center gap-2 py-2.5 px-4 rounded-[10px] border-[1.5px] border-gray-200 bg-white text-[13px] font-medium text-[#0F172A] hover:border-gray-400 hover:bg-gray-50 transition-all disabled:opacity-50 disabled:cursor-not-allowed">
                {icon} {label}
              </button>
            ))}
          </div>
          ...
          <form onSubmit={handleSubmit} noValidate aria-label="Formulaire d'inscription">
            ...
            <div className="mb-3.5">
              ...
              <div className="relative">
                ...
                <button
                  ...
                  className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-[#1565C0] transition-colors p-1 flex items-center"
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-3.5 rounded-[10px] text-white font-display text-[15px] font-bold transition-all duration-300 flex items-center justify-center gap-2.5 disabled:opacity-75"
              style={{ background: '#FF6B35' }}
              onMouseEnter={(e) => { if (!loading) { (e.currentTarget as HTMLButtonElement).style.background = '#E55A2B'; (e.currentTarget as HTMLButtonElement).style.boxShadow = '0 8px 28px rgba(255,107,53,.4)'; (e.currentTarget as HTMLButtonElement).style.transform = 'translateY(-1px)'; }}}
          ...
          <p className="text-center mt-4 text-sm text-[#64748B]">
            Déjà un compte ?{' '}
            <Link href="/auth/login" className="text-[#1565C0] font-semibold hover:text-[#FF6B35] transition-colors">Se connecter</Link>
          </p>
          <p className="text-center mt-3.5 text-[11px] text-gray-400 leading-relaxed">
